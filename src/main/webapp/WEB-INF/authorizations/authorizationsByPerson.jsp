...
<div id="authorizationList" class="col-lg-8" style="margin-top: 15px">
	...
	<c:forEach var="group" items="${groups}">
		<div id="period"  class="authorization period">
			...
			<div id="${group.key.externalId}" class="small">
				<table class="table" id="${group.key.username}">
			  	  <thead>
			  			<tr>
			  		...
			  		<tbody>
				  		<c:forEach var="auth" items="${group.value}">
				  			<tr class="auth ui-droppable" id="${auth.externalId}">
				  				<td>
				  					<button data-user-name="${group.key.username}" data-auth-id="${auth.externalId}" data-auth-name="${auth.operation.localizedName}"  data-toggle="modal" data-target="#confirmDeleteRule" class="btn btn-default" title=<spring:message code="label.delete"/>>
